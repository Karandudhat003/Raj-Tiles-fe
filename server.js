...
app.get("/", (req, res) => {
  res.json({
    message: "🚀 Raj Tiles Server Running with User Isolation",
    mongodb: mongoose.connection.readyState === 1 ? "Connected" : "Disconnected",
    time: new Date(),
    info: "Each sales user can only see their own data"
  });
});
...
app.listen(PORT, () => {
  console.log(`🚀 Server running at http://localhost:${PORT}`);
  console.log(`🔐 User isolation enabled - each sales user sees only their own data`);
  console.log(`👥 Test users:`);
  console.log(`   - user1 / password123 (sales)`);
  console.log(`   - user2 / password123 (sales)`);
  console.log(`   - admin / admin123 (admin - sees all data)`);
});
