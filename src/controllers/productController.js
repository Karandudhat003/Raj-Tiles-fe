...
// 🔥 ADD PRODUCT - Auto-assign to logged-in user
exports.addProduct = async (req, res) => {
  try {
    const data = req.body;
...
// 🔥 GET ALL PRODUCTS - Only for logged-in user (sales see only their products)
exports.getProducts = async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    console.log(`📥 User ${username} (${userRole}) fetching products...`);

    let query = {};
    
    // 🔥 Sales users see only their own products
    ...
    console.log(`✅ User ${username} found ${transformedProducts.length} products`);
...
// 🔥 GET PRODUCT BY ID - Only if user owns it (or is admin)
exports.getProductById = async (req, res) => {
  try {
    const userId = req.user.id;
    const userRole = req.user.role;
    ...
    if (userRole === "sales" && product.createdBy.toString() !== userId) {
...
// 🔥 UPDATE PRODUCT - Only if user owns it (or is admin)
exports.editProduct = async (req, res) => {
  try {
    const data = req.body;
    const userId = req.user.id;
    ...
    if (userRole === "sales" && existingProduct.createdBy.toString() !== userId) {
      return res.status(403).json({
        success: false,
...
// 🔥 DELETE PRODUCT - Only if user owns it (or is admin)
exports.deleteProduct = async (req, res) => {
  try {
    const userId = req.user.id;
    const userRole = req.user.role;
